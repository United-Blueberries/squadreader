...
def cmd_watch(args: argparse.Namespace) -> int:
    ...
    period = 1.0 / args.hz
    # Cache-hygiene windows are tick-count knobs; derive them from --hz so
    # the wall-clock guarantees (~30 s "every slot re-verified / re-derived")
    # hold at any tick rate instead of shrinking as the rate goes up.
    cache_window = max(60, round(30 * args.hz))
    caches.walk_resync_ticks = max(30, round(30 * args.hz))
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    # Nothing configured logging before, so every log.info() in the recorder
    # was silently dropped — including "recording: started match", "idle:
    # matchState=..." and "discarded short recording". When three matches went
    # unrecorded there was no way to find out why, because the code had been
    # explaining itself to a logger with no handler the whole time.
    # SQREADER_LOG_LEVEL overrides for a quieter/noisier run.
    logging.basicConfig(
        level=os.environ.get("SQREADER_LOG_LEVEL", "INFO").upper(),
    ...
    # --- unlicensed servers: derive a matchId Squad never assigned -----------
    # An unlicensed custom server leaves SQGameState.MatchID empty for the whole
    # match, which silently disables the recorder AND the stats store. Clan
    # servers have no reason to hold an OWI license, so this is a supported
    # deployment, not a misconfiguration. Inert on licensed boxes: a real id is
    # always returned untouched. See sqreader/synth_match.py.
    synth_ids = None
    if config.get("synthetic_match_ids"):
        _synth_dir = (stats_db_path.parent if stats_db_path
                      else (recordings_dir or Path.cwd()))
        try:
            from .synth_match import STATE_FILENAME, SyntheticMatchIds
            synth_ids = SyntheticMatchIds(
                _synth_dir / STATE_FILENAME,
                community=(push_creds or {}).get("SQREADER_COMMUNITY", ""),
                server_id=args.server_id)
        except Exception as _sy_e:
            print(f"synth-match init failed (unlicensed servers will not "
                  f"record): {_sy_e!r}", file=sys.stderr)
    ...
    # Set once a newer release has been staged; the tick loop then waits for a
    # moment with no match running and exits so systemd restarts us into it.
    _restart_for_version: str | None = None
    if _offset_state_dir is not None:
        # An applied-but-unconfirmed release is confirmed HERE — by the new
        # binary actually reaching this line. Clearing the marker at install
        # time (the first implementation) meant a release that crash-looped
        # left nothing behind saying an update had ever been attempted.
        from . import updater as _upd
        _confirm = _upd.confirm_boot(_offset_state_dir, __version__)
        if _confirm:
            print(_confirm, file=sys.stderr)
    ...
    def _check_update() -> None:
        ...
        try:
            from . import updater
            # Nothing to swap from a source checkout, so nothing to download.
            # Staging anyway would loop: the install step clears the stage, the
            # next check-in re-accepts the same release, and the agent restarts
            # for it every check-in interval.
            if updater.running_binary(compiled=_is_compiled()) is None:
                return
            got = updater.fetch_and_accept(
            ...
            if data is None:
                print(f"[update] release v{got['version']} download/verify failed",
                      file=sys.stderr)
                return
            updater.stage(_offset_state_dir, got["version"], got["artifact"],
                          data, sha256=got["sha256"])
            nonlocal _restart_for_version
            _restart_for_version = got["version"]
            print(f"[update] staged agent release v{got['version']} — will "
                  f"restart to apply once no match is in progress",
                  file=sys.stderr)
    ...
    if two_tier and record_hz is not None:
        ...
        try:
            ...
            # Degrade to the single-tier loop rather than dying. The reader's
            # job is to keep recording; losing the smoother position stream is
            # a downgrade, but a failed spawn used to raise straight out of
            # cmd_serve and kill the agent — under Restart=always that is a
            # fork/exec storm every RestartSec on someone's game server.
            print(f"  two-tier worker could not start ({exc}); "
                  f"falling back to single-tier recording", file=sys.stderr)
            worker = None
            two_tier = False
            sample_positions = None
            write_position_frame = None
        else:
    ...
    # Derived from --hz so the wall-clock guarantee (~30 s) holds at any
    # tick rate -- same reasoning as cmd_watch.
    CACHE_WINDOW_TICKS = max(60, round(30 * args.hz))
    caches.walk_resync_ticks = max(30, round(30 * args.hz))
    ...
    def _consume_full(snap: dict) -> None:
        ...
        # FIRST, before the frame is serialized: on an unlicensed server Squad
        # leaves matchId empty, so fill in a derived one here and every consumer
        # below — tee, .sqrx, recorder, stats, and the check-in's live block,
        # which reads this same dict — sees one consistent id.
        if synth_ids is not None:
            try:
                synth_ids.apply(snap)
            except Exception as _sy_e:
    ...
    try:
        while not stop["flag"]:
            ...
            if push_active and now - last_checkin >= fleet.CHECKIN_INTERVAL_SEC:
                ...
                _check_update()
                # Apply a staged release by exiting: the unit is Restart=always,
                # so systemd brings us straight back and ExecStartPre performs
                # the swap between the two processes.
                #
                # Not mid-match. A restart during a live game splits the replay
                # and was, in an earlier version, enough to publish the match
                # early — so the update waits for the game to be between
                # matches, however long that takes.
                if _restart_for_version and record_state_box.get("current") is None:
                    gs_now = (snap.get("gameState") or {}) if snap else {}
                    if gs_now.get("matchState") != "InProgress":
                        print(f"[update] no match in progress — restarting to "
                              f"apply v{_restart_for_version}", file=sys.stderr)
                        raise SystemExit(0)
                last_checkin = now
        ...
        if stats_store is not None:
            try:
                # A SHUTDOWN IS NOT A MATCH ENDING. Finalizing here marks the
                # row `final` and fires the hook that queues the match for
                # central, so restarting mid-match published it with only the
                # elapsed duration — an 18-second "draw" — and the half-written
                # .sqrx the recorder had just closed as `unverified` was
                # accepted as that match's replay. The archive then offered
                # "Watch" on a game still being played.
                #
                # Leave the row OPEN instead. `_open_match` re-opens the same
                # match_id on the next boot and does NOT touch `started_at`, so
                # the match keeps accumulating across the restart and is
                # published exactly once, when it actually ends, with its true
                # duration. The recorder still closes its writer above — that
                # is about writing a valid .sqrx footer, not about the match.
                gs = snap.get("gameState") or {}
                if gs.get("matchState") != "InProgress":
                    stats_store.finalize_open_match(reason="serve shutdown")
                else:
                    print("stats: match still InProgress — leaving it open for "
                          "the next boot instead of publishing it early",
                          file=sys.stderr)
                stats_store.close()
            except Exception as _e:
...
def cmd_enroll(args: argparse.Namespace) -> int:
    ...
    """
    from . import agent_creds, build_config, hwfp, ingest_client

    locked = build_config.locked_base()
    ...
    if args.status:
        ...
        if locked:
            print(f"  build:     locked to {locked}")
    ...
    if not args.token:
        print("usage: sqreader enroll <token> [--central-url URL]  (or --status)",
              file=sys.stderr)
        return 2
    requested = args.central_url or config.get("central_url")
    if locked:
        # A released binary enrolls against the central it was built for, full
        # stop. Warn rather than fail when someone passes a different URL — the
        # enrollment still succeeds against the right platform.
        if requested and not build_config.same_origin(requested, locked):
            print(f"note: this build is locked to {locked} — "
                  f"ignoring --central-url {requested}", file=sys.stderr)
        central = locked
    else:
        central = requested
        if not central:
...
def cmd_build_worker(args: argparse.Namespace) -> int:
    ...
    tick = 0
    bad_streak = 0
    # Same time-based hygiene windows as watch/serve, sized from the
    # parent's full-frame rate (passed down at spawn).
    cache_window = max(60, round(30 * args.hz))
    caches.walk_resync_ticks = max(30, round(30 * args.hz))
...
def _worker_prefix() -> list[str]:
    """How to re-invoke this program to run the `build-worker` subprocess.

    From source that is ``<python> -m sqreader.cli build-worker``. From a
    compiled build it is NOT: inside a Nuitka onefile ``sys.executable`` points
    at a ``python3`` in the throwaway extraction directory that does not exist,
    so spawning it raised FileNotFoundError and took the whole agent down on
    startup — a crash loop, since two-tier recording is set up before the tick
    loop begins. Nuitka exports the real artifact path as NUITKA_ONEFILE_BINARY;
    argv[0] is the fallback.
    """
    if _is_compiled():
        exe = os.environ.get("NUITKA_ONEFILE_BINARY") or sys.argv[0]
        return [exe, "build-worker"]
    return [sys.executable, "-m", "sqreader.cli", "build-worker"]


def cmd_version(args: argparse.Namespace) -> int:
    """Print the build identity — the installer's sanity check.

    ``install.sh`` runs this straight after placing the binary: it proves the
    artifact actually executes on this box (right glibc, crypto extension
    loaded) *before* units are written and a token is burned. Machine-readable
    with --json so the update path can compare versions without parsing prose.
    """
    import platform

    from . import __version__, build_config
    ...
    if getattr(args, "json", False):
        ...
        return 0
    print(f"sqreader {info['version']} ({info['platform']}, "
          f"{'compiled' if info['compiled'] else 'source'})")
    if locked:
        print(f"  central: {locked}")
    return 0


def cmd_retention(args: argparse.Namespace) -> int:
    """Prune old recordings so the disk cannot fill.

    Recordings grow by hundreds of MB a day and nothing else deletes them. On a
    box that also runs the game server a full disk takes SQUAD down too, so this
    is a safety valve, not housekeeping.

    Ported from deploy/cleanup_recordings.sh because a compiled agent must not
    depend on a shell script sitting next to it.

    Safety rules, in force for every policy:
      * a file modified within --in-progress-min minutes is the match being
        recorded RIGHT NOW — never touched, deleting it corrupts a live capture;
      * a .sqrx is removed together with its .meta.json sidecar;
      * --dry-run changes nothing.

    Exits 0 even when nothing matched, so the systemd timer stays green.
    """
    import shutil
    import time

    d = Path(args.recordings_dir).expanduser()
    if not d.is_dir():
        print(f"recordings dir not found: {d}", file=sys.stderr)
        return 1
    dry = bool(args.dry_run)
    if dry:
        print("[retention] DRY RUN — nothing will be deleted")

    cutoff = time.time() - args.in_progress_min * 60
    files = []
    for p in d.glob("*.sqrx"):
        try:
            st = p.stat()
        except OSError:
            continue
        if st.st_mtime > cutoff:          # the in-flight recording
            continue
    ...
    if args.max_age_days:
        age_cut = time.time() - args.max_age_days * 86400
        for mtime, _sz, p in files:
            if mtime < age_cut:
                mark(p)

    live = sum(sz for _m, sz, p in files if p not in doomed)
    if args.max_total_gb:
        limit = args.max_total_gb * 1024**3
    ...
    if args.min_free_gb:
        ...
        try:
            ...
            free = need
        freed = sum(sz for _m, sz, p in files if p in doomed)
        # Always keep the newest few, whatever the numbers say. Free space is a
        # property of the WHOLE filesystem but recordings are the only thing
        # this can delete, so on a box where the disk is full for another reason
        # (the game server's own install, another service) the target is
        # unreachable and the loop below would delete every match ever recorded
        # and still not reach it.
        keepers = {p for _m, _sz, p in files[-max(args.min_keep, 0):]} \
            if args.min_keep else set()
        deletable = sum(sz for _m, sz, p in files
                        if p not in doomed and p not in keepers)
        if free + freed + deletable < need:
            print(f"[retention] cannot reach {args.min_free_gb:g} GB free by "
                  f"pruning recordings (only {deletable / 1024**3:.1f} GB here, "
                  f"{free / 1024**3:.1f} GB free) — something else is filling "
                  f"the disk; not deleting on this policy", file=sys.stderr)
        else:
            for _m, sz, p in files:
                if free + freed >= need:
                    break
    ...
    for p in doomed:
        try:
            ...
            pass
        print(f"[retention] {'would remove' if dry else 'removing'} {p.name}")
        if dry:
            continue
        for victim in (p, p.with_suffix(".meta.json")):
            try:
                victim.unlink()
            except FileNotFoundError:
                pass
            except OSError as exc:
                print(f"[retention] could not remove {victim}: {exc}",
                      file=sys.stderr)
    print(f"[retention] {len(doomed)} recording(s), "
          f"{total / 1024**3:.2f} GB {'would be' if dry else ''} freed")
    return 0


def cmd_download_auth(args: argparse.Namespace) -> int:
    """Print the X-Sqr-Auth header value for downloading `file` from central.

    Used by install.sh's upgrade path on an already-enrolled box. Writes ONLY
    the compact JSON envelope to stdout so it can be dropped straight into a
    header; every diagnostic goes to stderr, and any credential problem exits
    non-zero — so a caller under `set -e` fails cleanly instead of splicing a
    stray line into the header.
    """
    from . import agent_creds, ingest_client

    creds = agent_creds.load()
    if not creds or not creds.get("SQREADER_AGENT_ID") \
            or not creds.get("SQREADER_AGENT_SECRET_HEX"):
        print("not enrolled — no credentials to sign a download with",
              file=sys.stderr)
        return 1
    try:
        sys.stdout.write(ingest_client.download_auth_header(creds, args.file))
        sys.stdout.write("\n")
    except Exception as exc:                              # noqa: BLE001
        print(f"could not build download auth: {exc}", file=sys.stderr)
        return 1
    return 0


def cmd_selftest(args: argparse.Namespace) -> int:
    """Prove this build actually works on this machine.

    A binary that starts is not a binary that works: the fragile parts of the
    bundle are cryptography's Rust extension against the system OpenSSL, the
    zstandard C extension, and whether the embedded ``data/static`` tables were
    really included. All three fail *late* and quietly otherwise — enrolment
    dies mid-onboarding, replays fail to seal, or maps render blank with
    nothing in the log. The build smoke-tests with this, and install.sh runs it
    before writing units or burning a token.
    """
    from .squad.metadata import DEFAULT_DATA_DIR
    ...
    def _zstd() -> str:
        import zstandard
        blob = zstandard.ZstdCompressor().compress(b"sqreader" * 64)
        assert zstandard.ZstdDecompressor().decompress(blob) == b"sqreader" * 64
        return f"{len(blob)} B round-trip"

    def _aead() -> str:
        from cryptography.hazmat.primitives.ciphers.aead import AESGCM
        key, nonce = os.urandom(32), os.urandom(12)
        ct = AESGCM(key).encrypt(nonce, b"payload", b"aad")
        assert AESGCM(key).decrypt(nonce, ct, b"aad") == b"payload"
        return "AES-256-GCM"

    def _sig() -> str:
        from cryptography.hazmat.primitives.asymmetric import ed25519
        sk = ed25519.Ed25519PrivateKey.generate()
        sk.public_key().verify(sk.sign(b"msg"), b"msg")
        return "Ed25519"

    def _data() -> str:
        d = Path(DEFAULT_DATA_DIR)
        if not d.is_dir():
            raise FileNotFoundError(f"data/static not found at {d}")
        n = len(list(d.glob("*.json")))
        if n == 0:
            raise FileNotFoundError(f"no metadata tables under {d}")
        return f"{n} tables at {d}"

    print("sqreader selftest")
    check("compression", _zstd)
    check("sealed envelope", _aead)
    check("release signatures", _sig)
    check("bundled metadata", _data)
    print("PASS" if ok else "FAIL")
    return 0 if ok else 1


def cmd_apply_staged_update(args: argparse.Namespace) -> int:
    """Install a staged agent release, and take it back if it will not run.

    Runs from the unit's ExecStartPre — between the old process stopping and the
    new one starting, which is the only moment a swap is uneventful. It is NOT
    run inside the live reader.

    The previous implementation pip-installed the artifact. That could never
    work: what is shipped is a single self-contained executable, there is no
    wheel and no site-packages to install into, and inside a Nuitka onefile
    `sys.executable` points into a throwaway extraction directory — a fact this
    file already documents for the build-worker path.

    Three things make this safe to run unattended:
      * the candidate must execute (`version`) before it is allowed to take over
      * the outgoing binary is kept as `.bak`
      * the marker SURVIVES the swap. It is cleared only once the new version
        has actually come up and said so. A release that crash-loops is
        therefore detectable, and after MAX_BOOT_ATTEMPTS it is rolled back.
    """
    from . import updater
    if args.state_dir:
    ...
    target = updater.running_binary(compiled=_is_compiled())
    if target is None:
        print("staged update present but this is a source checkout — nothing "
              "to swap; clearing", file=sys.stderr)
        updater.clear_staged(state_dir)
        return 0

    # Already swapped on an earlier boot and still not confirmed: the new
    # binary is not coming up. Count the attempt, then put the old one back.
    if st.get("applied_at"):
        n = updater.note_boot_attempt(state_dir)
        if n <= updater.MAX_BOOT_ATTEMPTS:
            print(f"[update] v{st.get('version')} applied but unconfirmed "
                  f"(boot {n}/{updater.MAX_BOOT_ATTEMPTS})", file=sys.stderr)
            return 0
        ok, msg = updater.rollback_binary(target)
        print(f"[update] v{st.get('version')} failed to come up after "
              f"{n} boots — rolling back: {msg}", file=sys.stderr)
        updater.mark_refused(state_dir, f"did not boot in {n} attempts: {msg}")
        return 0 if ok else 1

    ok, msg = updater.swap_binary(st.get("path", ""), target,
                                  sha256=str(st.get("sha256") or ""))
    if not ok:
        print(f"[update] refusing v{st.get('version')}: {msg}", file=sys.stderr)
        # Refused, not forgotten. Forgetting it means the next check-in accepts
        # the same release, stages it, and the agent exits to apply it again —
        # a permanent restart loop across the whole fleet on one bad build.
        updater.mark_refused(state_dir, msg)
        return 1
    updater.mark_applied(state_dir)
    print(f"[update] installed v{st.get('version')} ({msg}); "
          f"previous binary kept at {target.name}{updater.BACKUP_SUFFIX}",
          file=sys.stderr)
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_bw.add_argument(
        "--hz", type=float, default=3.0,
        help="parent's full-frame rate; sizes the cache-hygiene windows "
             "(~30 s wall-clock) like watch/serve")
    ...
    p_ver = sub.add_parser("version",
                           help="print build version/identity (installer check)")
    ...
    p_self = sub.add_parser(
        "selftest",
        help="verify crypto, compression and bundled data work on this box")
    p_self.set_defaults(func=cmd_selftest)

    p_da = sub.add_parser(
        "download-auth",
        help="(internal) print the X-Sqr-Auth header for a gated download")
    p_da.add_argument("file", help="artifact filename being downloaded")
    p_da.set_defaults(func=cmd_download_auth)

    p_ret = sub.add_parser(
        "retention",
        help="prune old recordings (run by the daily timer; a full disk on a "
             "game box takes Squad down too)")
    p_ret.add_argument("--recordings-dir", type=Path, required=True)
    p_ret.add_argument("--max-age-days", type=float, default=0,
                       help="drop recordings older than N days (0 = off)")
    p_ret.add_argument("--max-total-gb", type=float, default=0,
                       help="drop oldest until the dir fits in N GB (0 = off)")
    p_ret.add_argument("--min-free-gb", type=float, default=0,
                       help="drop oldest until the filesystem has N GB free")
    p_ret.add_argument("--in-progress-min", type=float, default=60,
                       help="never touch a file modified in the last N minutes "
                            "— that is the match recording right now")
    p_ret.add_argument("--min-keep", type=int, default=3,
                       help="always keep at least this many of the newest "
                            "recordings, whatever the free-space policy says")
